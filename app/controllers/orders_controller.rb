  ...
  def create
    ...
    respond_to do |format|
      if @order.save
        ...
        session[:cart_id] = nil
 		OrderNotifier.received(@order).deliver_now		#отправка почтового уведомления сразу
#		OrderNotifier.delay.received(@order)		        #отправка cooбщения в фоновом режиме `rake jobs:work`
